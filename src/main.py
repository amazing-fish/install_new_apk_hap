...
class App(tk.Tk):
    ...
    def fetch_crash_log(self) -> None:
        ...
        if not device:
            ...
            return
        if device.platform not in {"android", "harmony"}:
        ...
        self.log(f"开始获取崩溃日志: {device_id}({device.platform}) -> {log_path}")
        threading.Thread(
            target=self._fetch_crash_log_worker,
            args=(device, log_path),
            daemon=True,
        ).start()

    def _fetch_crash_log_worker(self, device: DeviceInfo, log_path: Path) -> None:
        if device.platform == "android":
            result = run_android_dropbox_dump(device.device_id, log_path)
        else:
            result = run_harmony_hilog_dump(device.device_id, log_path)
        self.after(
            0,
            self._apply_crash_log_result,
            device.device_id,
            device.platform,
            log_path,
            result.command,
    ...
    def _apply_crash_log_result(
        ...
        self.log(f"{platform.capitalize()} {device_id} 崩溃日志命令: {' '.join(command)}")
        if returncode != 0:
        ...
        self.log(f"获取崩溃日志成功：设备 {device_id}({platform})，输出已追加到 {log_path}")
