...
from ui_layout import build_main_layout
...
class App(tk.Tk):
    _DEVICE_LIST_MAX_ROWS = 4

    def __init__(self) -> None:
        super().__init__()
        self.title("APK/HAP 安装控制台")
        ...
        self.device_summary_var = tk.StringVar(value="未刷新设备")
        self.selected_device_var = tk.StringVar(value="未选择设备")
        self.package_summary_var = tk.StringVar(value="等待选择安装包目录")
    ...
    def _single_selected_device(self) -> Optional[DeviceInfo]:
        selection = self.device_tree.selection()
        if len(selection) != 1:
            return None
        selected_id = selection[0]
        return next((device for device in self.devices if device.device_id == selected_id), None)

    def _update_action_availability(self) -> None:
        device = self._single_selected_device()
        is_connected = device is not None and device.status.lower() in {"device", "connected"}
        is_harmony = is_connected and device is not None and device.platform == "harmony"
        is_known_platform = is_connected and device is not None and device.platform in {"android", "harmony"}

        self.udid_button.config(
            state=tk.DISABLED if self.udid_fetching or not is_harmony else tk.NORMAL
        )
        log_state = tk.DISABLED if self.crash_log_fetching or not is_known_platform else tk.NORMAL
        nextdemo_state = tk.DISABLED if self.crash_log_fetching or not is_harmony else tk.NORMAL
        self.crash_log_button.config(state=log_state)
        self.nextdemo_log_button.config(state=nextdemo_state)
    ...
    def _apply_device_refresh(self, devices: List[DeviceInfo]) -> None:
        ...
        if only_device_id:
            ...
            restored_selection = [device_id for device_id in previous_selection if device_id in current_device_ids]
            if restored_selection:
                self.device_tree.selection_set(restored_selection)
            self.name_var.set("")
        self._update_device_summaries()
        self._update_action_availability()
    ...
    def save_device_name(self) -> None:
        ...
        self.device_tree.set(device_id, "name", self._device_label(device_id))
        self._update_device_summaries()
