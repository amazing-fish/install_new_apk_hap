...
- **应用名称**：`package_metadata` 只读明确 label 字段，APK 使用可选 AAPT2，HAP 字面 label 直接读取、编译字符串使用可选 restool 默认资源值；工具发现、格式及读取上限详见 README。不执行包内代码，不递归猜测任意 name，不把资源引用或文件名称为解析成功。`package_label_loader` 按文件/工具指纹缓存至多 256 项，丢弃过期结果。下拉显示字符串防碰撞，仍映射真实 Path；名称更新保持选择及安装快照，-t 仍按文件名存储，无新增说明行或逐包日志。
...
- **自动化打包**：GitHub Actions 在 Windows 环境使用 PyInstaller 生成 exe，可手动触发或打 tag；tag 触发时会将 exe 上传到 release assets。
  - 当前候选版本 `v0.8.0`；分支手动构建的版本输入必须匹配 `VERSION`，产物仍需按运行 SHA 区分。PR 交付不等于正式 tag/Release 发布。
