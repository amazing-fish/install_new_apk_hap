...
- **崩溃日志采集**：
  - Android：设备列表“获取崩溃日志”按钮执行 `adb -s <device_id> shell dumpsys dropbox --print`，并将输出追加写入 `D:\crash.log`。
  - Harmony（含 HarmonyOS 6）：通过 `hdc -t <device_id> shell hilog -x` 导出日志缓冲区并追加写入 `D:\crash.log`；用于采集崩溃相关日志时，需结合关键字（如 `FATAL`、`crash`）筛选。
