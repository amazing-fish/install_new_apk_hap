...
- **应用名称解析展示**：扫描安装包后会尝试解析应用名称（APK/HAP），并在安装按钮上方一行展示“应用名称”；下拉框展示“应用名称（文件名）”，解析失败自动回退文件名。
- **HAP `module.json5` 解析稳健性**：解析 `label/appName/name` 时按 `key: "value"` 结构提取冒号后的值，避免将 key 片段误拼入应用名称。
- **HAP `module.json/pack.info` 嵌套元数据解析**：JSON 分支支持递归遍历对象与数组，兼容 `module.name` 等嵌套结构，降低误回退到文件名的概率。
