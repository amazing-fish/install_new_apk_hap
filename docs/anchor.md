...
- **视觉配置**：`ui_styles.py` 集中管理默认窗口、设备行数上限、下拉可视行数、列宽/标题与新设备高亮。保留平台原生 ttk 主题和现有几何布局，不加入全局主题覆盖。
...
- **状态显示**：表格视觉顺序为名称、平台、状态、设备码，行身份仍为 `device_id`，状态保留探测原值。`ui_display.py` 统一名称、平台和摘要格式化；统计表示当前检测列表，不将未授权/离线设备宣称为可安装。
  ...
  - 保留默认 500x600 窗口，通过设备区标题承载统计和收紧段间距容纳摘要；布局及摘要换行绑定由 `ui_layout.py` 维护，多设备/超长内容的整体滚动由后续阶段单独处理。
