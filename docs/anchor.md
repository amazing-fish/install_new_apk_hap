...
- **自动化打包**：GitHub Actions 在 Windows 环境使用 PyInstaller 生成 exe，可手动触发或打 tag；tag 触发时会将 exe 上传到 release assets。
  - 当前候选版本 `v0.8.1`；分支手动构建的版本输入必须匹配 `VERSION`，产物仍需按运行 SHA 区分。PR 交付不等于正式 tag/Release 发布。
...
- `src/services/package_metadata.py`：有界元数据读取、SDK 名称工具与显示状态
