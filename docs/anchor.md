...
- **线程策略**：设备刷新、安装前探测、UDID 获取与包名称解析在后台线程执行。包目录枚举和按修改时间排序仍在主线程；元数据使用单个合并请求的 worker + 有界结果队列，Tk after 仅消费当前目录/请求且指纹一致的结果；窗口销毁关闭 worker 与轮询。
- **设备探测**：
...
- **应用名称**：`package_metadata` 只读明确 label 字段，Windows x64 exe 内置 AAPT2/RestoolV2，源码运行使用可选外部 SDK；HAP 字面 label 直接读取、编译字符串取 restool 默认资源值；工具发现、格式及读取上限详见 README。ZIP 目录保持 1 MiB 字节预算，不额外以 10,000 项误拦正常 APK；SDK 非零退出单独显示工具解析失败，后续刷新重试，资源格式需要兼容版本的工具。不执行包内代码，不递归猜测任意 name，不把资源引用或文件名称为解析成功。`package_label_loader` 按文件/工具指纹缓存至多 256 项，丢弃过期结果。下拉显示字符串防碰撞，仍映射真实 Path；名称更新保持选择及安装快照，-t 仍按文件名存储，无新增说明行或逐包日志。
- **联动刷新**：界面的“刷新设备”和“扫描最新包”按钮都会触发设备检测与当前安装包目录扫描，设备刷新期间两个入口同步禁用。
- **Harmony 部分探测失败**：刷新日志保留原因和可用 Android 数量，保留仍可用的原选择但不自动改选其他设备；恢复后首次普通刷新重新记录成功，随后继续去重。安装前若未选择设备、原选择包含 Harmony 或无法确认的平台则停止；仅已明确选择的纯 Android 请求可继续，保留校验完成/耗时日志，避免自动转移到其他设备。
...
- **自动化打包**：GitHub Actions 通过 `scripts/build_exe.py` 校验并打入固定版本的名称解析工具及完整 NOTICE，再运行 `scripts/verify_exe.py`，将单个 exe 复制至无 SDK 配置的临时环境，验证 APK/HAP v1/v2、内置哈希与许可导出后才上传。可手动触发或打 tag；tag 触发时会将通过验收的 exe 上传到 release assets。
  - 当前候选版本 `v0.8.2`；分支手动构建的版本输入必须匹配 `VERSION`，产物仍需按运行 SHA 区分。PR 交付不等于正式 tag/Release 发布。
...
- `src/services/package_metadata.py`：有界元数据读取、内置/外部名称工具与显示状态
- `src/services/package_metadata_cli.py`：最终 exe 的无 GUI 读名报告及许可导出
- `vendor/metadata-tools/`：固定版本工具、来源、哈希与完整 NOTICE
- `src/services/package_label_loader.py`：单 worker、请求合并、文件/工具指纹缓存，不依赖 Tk
