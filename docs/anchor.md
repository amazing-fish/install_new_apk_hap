...
- **视觉配置**：`ui_styles.py` 集中管理默认 720x780、最小 480x560 窗口、设备行数上限、下拉可视行数、列宽/标题与新设备高亮，保留平台原生 ttk 主题。设备列根据实际文本测量宽度，完整内容通过水平滚动访问。
- **可达性**：设备、安装包、目标确认、日志四区位于 `ScrollableArea`，底部安装/中止按钮和紧凑状态摘要固定。完整选择/包摘要在目标确认区换行；不让长摘要撑高底栏。`ActionRow` 按按钮实际请求宽度换行。滚轮绑定仅限当前窗口，设备表/下拉/日志保留自身滚动；Tab 焦点自动显露，Ctrl+Home/End 与 Ctrl+PageUp/PageDown 控制页面，销毁容器时清理绑定。
