...
- 可视化界面
- 分步工作区与固定安装/中止入口，支持窄窗口滚动、长内容横向查看和键盘操作
...
界面默认 720x780，最小 480x560。按“选择设备 → 选择安装包 → 确认目标 → 日志”操作，底部安装/中止按钮始终可见。
长设备名称/设备码可通过表格横向滚动查看；下拉框保留全部安装包候选，选中包完整名称显示在目标确认区。日志可横纵滚动。
